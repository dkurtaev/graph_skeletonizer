...
TEST(OptimalMethods, equals_costs) {
  ...
  for (unsigned iter = 0; iter < kNumGenerations; ++iter) {
    ...
    float boruvka_spanning_tree_cost = Graph::WeightsSum(spanning_tree);

    KruskalMethod::Process(n_nodes, edges, &spanning_tree);
    float kruskal_spanning_tree_cost = Graph::WeightsSum(spanning_tree);

    PrimMethod::Process(n_nodes, edges, &spanning_tree);
    float prim_spanning_tree_cost = Graph::WeightsSum(spanning_tree);
